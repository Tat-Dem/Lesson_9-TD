...
test('Successful order update with valid API key and id and return status code OK', async  ({ request }) => {
...
test('Unsuccessful update of order with valid API key and invalid id and return status code Not Found', async  ({ request }) => {
  const requestHeaders ={
    api_key: '1234567890123456',
  }
  const requestBody = {
...
test('Unsuccessful update of order with missing API key and return status code Unauthorized', async  ({ request }) => {
  const requestBody = {
    status: 'OPEN',
    courierId: 0,
    customerName: 'Harry',
